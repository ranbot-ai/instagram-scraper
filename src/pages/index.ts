...
const requeueFailedRequest = async (page: any, queueItem: any, queue: any, identifier: any) => {
  queueItem.tries += 1;
  if (queueItem.tries < config.max_tries) {
    queue.push(queueItem);
    console.log(" > Retry " + queueItem.tries + " for " + identifier.identifier);
  } else {
...
async function scrapeInstagramPublicPage(browser: any, queue: IQueueItem[]): Promise<boolean> {
  const cache: any = {};
  let stopping = false;
  ...
  while (queue.length > 0) {
    ...
    page.on('response', async(response: any) => {
      ...
      let skippedProxyDomainURL = new RegExp(zproxy.bypass_domains.join('|')).test(requestURL);

      if (responseSize === undefined || responseSize === '0') {
        return;
      } else {
        responseSize = parseInt(responseSize)
        if (!skippedProxyDomainURL) { proxyResponseSize += responseSize }
      }
      ...
      if (
        ...
        try {
          ...
          console.error("// Related profiles response interception error: ", error);
        }
      }
    })
    ...
    try {
      ...
      if (error instanceof Error && (error as Error).message.match(/net::ERR_CONNECTION_RESET/)) {
    ...
    const currentURL = await page.evaluate(() => decodeURIComponent(document.location.href));
    ...
    if (tempData) {
      data = tempData
    } else if (tempData === undefined && webProfileInfo) { // Sometimes, this request send multiple times, and after page parse
      data = wrapperWebProfileInfo(webProfileInfo) || data;
    }

    if (data) {
      data.response_size = { total: totalResponseSize, proxy: proxyResponseSize }

      // Get related profiles
      if (relatedProfileInfo) { data.edge_related_profiles = relatedProfileInfo.data.edge_chaining }
