...
async function parsePageData(page: any): Promise<any> {
  ...
  data.not_found = await page.evaluate((selector: string, notFoundMessage: string) => {
  ...
  }, mainSectionSelector, pageNotAvailable);
  ...
  if (config.automatically_detect_shared_data) {
    ...
    let sharedData = bodyHTML.match(/,"result":{"response":"{\\"data\\":(.*)}","status_code":200}}}}\]\]\]}\);}\);}\);<.script>/);
  ...
  let bodyHTML = await page.evaluate(() => document.body.innerHTML);
  let queryId = bodyHTML.match(/{"query":{"content_type":"PROFILE","target_id":"(\d+)"}}}}/);
  if (queryId !=null ) { data.id = parseInt(queryId[1]) }
  ...
  if (data.username == null) {
    ...
    if (queryUsername !=null ) { data.username = queryUsername[1] }
  }

  // Full Name
  // 'monday.com (@mondaydotcom) • Instagram photos and videos'
  // 'Ekohe (@weareekohe) • Instagram photos and videos'
  data.full_name = await page.evaluate(() => { return document.title.split(' • ')[0].replace(/ \(.*\)/, '') });
  ...
  data.external_url = externalLink && decodeURIComponent(externalLink.match(/https\:\/\/l\.instagram\.com\/\?u=([^&]+)&.*/)[1]);
  ...
  let relatedProfiles = await page.evaluate((buttonsSelector: string, chevronButtonAriaLabel: string) => {
    ...
    let xpathResult = document.evaluate("//div[contains(text(), 'Suggested')]", document, null, XPathResult.ANY_TYPE, null);
    let suggestedTitle = xpathResult.iterateNext();
    if (suggestedTitle) {
      let elements = suggestedTitle.parentElement?.parentElement?.parentElement?.querySelectorAll('li');

      if (elements) {
        let relatedProfiles = Array.from(elements).map(function(item) {
          ...
          if (fullNameElement instanceof HTMLElement) {
            return {
              node: {
                ...
                is_verified: (item?.querySelector('span[title="Verified"]') != null ? true : false),
    ...
    return [];
  }, buttonsSelector, chevronButtonAriaLabel);
  ...
  if (relatedProfiles.length === 0) {
    relatedProfiles = await page.evaluate((relatedProfilesSelector: string) => {
      ...
      let relatedProfiles = Array.from(accounts).map(function(item) {
        let fullNameElement = item?.querySelectorAll('span > div')[0];
