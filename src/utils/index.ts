  ...
  await page.evaluate(async (delay: number) => {
    await new Promise((resolve: any, reject: any) => {
      ...
      var timer = setInterval(() => {
        ...
        console.info("// " + Math.round(totalHeight) + "px out of " + scrollHeight + "px -> " + (Math.round((totalHeight/scrollHeight)*100)) + "%");
...
export { sleep, humanToNumber, autoScroll, asyncForEach, randomUrl, randomNumber }